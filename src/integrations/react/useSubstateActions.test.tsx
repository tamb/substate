  ...
  describe('Component integration', () => {
    test('should work correctly in React components', () => {
      function TestComponent() {
        ...
        const [count, setCount] = React.useState(store.getCurrentState().count);
        
        // Listen to store changes to update local state
        React.useEffect(() => {
          const handleUpdate = () => setCount(store.getCurrentState().count);
          store.on('STATE_UPDATED', handleUpdate);
          store.on('STATE_RESET', handleUpdate);
          return () => {
            store.off('STATE_UPDATED', handleUpdate);
            store.off('STATE_RESET', handleUpdate);
          };
        }, []);
